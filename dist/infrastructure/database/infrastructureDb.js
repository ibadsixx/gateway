...
exports.infrastructureDb = void 0;
...
class InfrastructureDatabase {
    ...
    async getProviderByName(name) {
        if (this.fallback)
            return this.fallbackProviders.find(p => p.name === name) || null;
    ...
    async getWritableProject(domain) {
        if (this.fallback) {
            const candidates = this.fallbackProjects.filter(p => p.domain === domain && p.write_enabled && p.status === 'active'
        ...
        if (error)
            throw new Error(`Failed to fetch writable project: ${error.message}`);
        const projects = data || [];
        if (projects.length === 0)
            return null;
        const healthy = projects.filter(p => p.capacity <= 0 || (p.used_space / p.capacity) * 100 < 90);
        return healthy[0] || projects[0];
    }
    async getProjectById(id) {
        if (this.fallback)
            return this.fallbackProjects.find(p => p.id === id) || null;
    ...
    async getProjectByKey(key) {
        if (this.fallback)
            return this.fallbackProjects.find(p => p.project_key === key) || null;
        const { data, error } = await this.client.from('infrastructure_projects').select('*').eq('project_key', key).single();
        if (error && error.code !== 'PGRST116')
            throw new Error(`Failed to fetch project by key: ${error.message}`);
        return data || null;
    }
    ...
    async updateProjectStatus(id, status) {
        const update = { status, last_health_check: new Date().toISOString() };
        if (this.fallback) {
    ...
    async updateProjectHealth(id, responseTime, usedSpace) {
        const update = {
    ...
    async collectUsageMetrics() {
        ...
        for (const project of projects) {
            ...
            if (!match || !project.management_pat)
                continue;
            ...
            try {
                ...
                console.warn(`[InfrastructureDB] Error querying ${project.project_key}:`, err.message);
            }
        }
    }
    // ---- Storage Providers ----
    ...
    async getActiveStorageAccounts() {
        const accounts = await this.getStorageAccounts();
        return accounts.filter(a => a.status === 'active');
    }
    async getStorageAccountById(id) {
        if (this.fallback)
            return this.fallbackStorage.find(a => a.id === id) || null;
        const { data, error } = await this.client.from('storage_providers').select('*').eq('id', id).single();
        if (error && error.code !== 'PGRST116')
            throw new Error(`Failed to fetch storage account: ${error.message}`);
        return data || null;
    }
    async registerStorageAccount(account) {
        if (this.fallback) {
            const row = {
                id: crypto.randomUUID(),
                ...account,
        ...
        if (error)
            throw new Error(`Failed to register storage account: ${error.message}`);
        return data;
    }
    ...
    async getDomain(name) {
        if (this.fallback)
            return this.fallbackDomains.find(d => d.name === name) || null;
    ...
    async getCurrentWriteProject(domain) {
        ...
        if (!domainRow || !domainRow.current_write_project)
            return null;
    ...
    async setCurrentWriteProject(domain, projectId) {
        if (this.fallback) {
            ...
            if (domainRow)
                domainRow.current_write_project = projectId;
            return;
        }
        const { error } = await this.client.from('domains').update({ current_write_project: projectId }).eq('name', domain);
        if (error)
            throw new Error(`Failed to set write project: ${error.message}`);
    }
    ...
    async getAllSettings() {
        if (this.fallback)
            return [...this.fallbackSettings.values()];
    ...
    async setSetting(key, value, type = 'string') {
        ...
        if (fetchError && fetchError.code !== 'PGRST116')
            throw new Error(`Failed to check setting: ${fetchError.message}`);
        if (existing) {
    ...
    async logHealth(entry) {
        ...
        if (error)
            throw new Error(`Failed to log health: ${error.message}`);
        return data;
    }
    async getHealthLogs(resourceType, resourceId, limit = 100) {
        if (this.fallback) {
            let logs = [...this.fallbackHealthLogs];
            if (resourceType)
                logs = logs.filter(l => l.resource_type === resourceType);
            if (resourceId)
        ...
        const { data, error } = await query;
        if (error)
            throw new Error(`Failed to fetch health logs: ${error.message}`);
        return data || [];
    }
...
exports.infrastructureDb = new InfrastructureDatabase();
//# sourceMappingURL=infrastructureDb.js.map